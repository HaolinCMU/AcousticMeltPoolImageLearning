...
class Model_VAE(object):
    """
    VAE training and evaluation framework. 
    """

    def __init__(self, input_data_dir, output_data_dir, batch_size, learning_rate, num_epochs, 
                 loss_beta=1., valid_ratio=.05, test_ratio=.15, model_arxiv_dir=ML_VAE.MODEL_ARXIV_DIR, 
        ...
        # Dataset & Dataloaders. 
        self.dataset = FrameAutoencoderDataset(self.input_data_dir, self.output_data_dir) # Torch.Dataset. Dataset object for initializing image dataset. 
        ...
        # Learning model. 
        self.vae_net = vae.VAE_Conv_test().to(self._device) # Torch.nn.Module. Initialize a VAE neural network object. Send to `device`.
    ...
    def valid_set_ind_array(self):
        ...
        else: raise ValueError("Validationdataloader partition and generation failed. ")
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for iter, batch in enumerate(self.train_loader):
                ...
                loss_train_perBatch_perEpoch = self._loss_func(y, groundtruths_train, mu, logvar) # Total loss of one batch. 
